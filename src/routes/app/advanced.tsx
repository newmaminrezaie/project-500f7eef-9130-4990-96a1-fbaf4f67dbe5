...
export const Route = createFileRoute("/app/advanced")({
  head: () => ({ meta: [{ title: "حالت پیشرفته — حسابداری زعفران رضایی" }] }),
  component: Advanced,
});
      ...
      <div className="grid grid-cols-4 gap-2">
        {DOC_TILES.map((t) => {
          ...
          return (
            ...
            >
              <div className={`grid h-14 w-14 place-items-center rounded-2xl ${t.bg} ${t.fg}`}>
                <Icon className="h-7 w-7" />
              </div>
              <div className="text-[11px] font-bold leading-tight text-foreground">{t.label}</div>
            </Link>
          );
        })}
        <Link
...
function Advanced() {
  return (
    <div className="space-y-4">
      ...
      >
        <ChevronRight className="h-4 w-4" /> بازگشت به صفحه ساده
      </Link>
      <Suspense
